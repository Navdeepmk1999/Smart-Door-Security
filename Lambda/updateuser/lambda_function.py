...
def lambda_handler(event, context):
    try:
        ...
        update_expr = []
        expr_names = {}  # Add this to handle reserved keywords
        expr_values = {}

        # Update name if provided
        if 'name' in body and body['name']:
            update_expr.append("#name = :n")  # Use an alias for the reserved keyword
            expr_values[':n'] = body['name']
            expr_names['#name'] = 'name'  # Map the alias to the actual attribute name
        ...
        photo_key = item.get('imageLocation', None)
        ...
        if 'photo' in body and body['photo']:
            ...
            if photo_key:
                ...
                try:
                    bucket_key = photo_key.split("/", 3)[-1]
                    s3.delete_object(Bucket=S3_BUCKET, Key=bucket_key)
        ...
        users_table.update_item(
            ...
            ExpressionAttributeNames=expr_names)  # Include the attribute name aliases
